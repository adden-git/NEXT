...
CHANGELOG_URL_ZH = "https://moonshotai.github.io/nexus-station/zh/release-notes/changelog.html"
CHANGELOG_URL_EN = "https://moonshotai.github.io/nexus-station/en/release-notes/changelog.html"
...
def _run_update_gate(current_version: str, latest_version: str) -> None:
    ...
    console.print(
        Panel(
            body,
            title="[bold]nexus-station update available[/bold]",
            border_style="yellow",
            expand=False,
            padding=(1, 2),
        )
    ...
    if key in ("\r", "\n"):
        ...
        if result.returncode == 0:
            console.print("[green]Upgrade complete! Run nexus-station to start the new version.[/green]")
        else:
            console.print("[red]Upgrade failed. Please try running manually:[/red]")
            console.print(f"  {UPGRADE_COMMAND}")
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        ...
        with tempfile.TemporaryDirectory(prefix="nexus-station-") as tmpdir:
    ...
    _print("[yellow]Restart NEXUS Station to use the new version.[/yellow]")
