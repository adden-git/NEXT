...
from nexus_station.wire import Wire
...
class WireServer:
    ...
    async def _handle_initialize(
        ...
        from nexus_station.hooks.config import HOOK_EVENT_TYPES
        from nexus_station.hooks.engine import WireHookHandle, WireHookSubscription
        from nexus_station.soul import wire_send
        from nexus_station.wire.protocol import WIRE_PROTOCOL_VERSION
        from nexus_station.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: NexusToolset) -> None:
        """Hide or unhide the AskUserQuestion tool based on client capabilities."""
        from nexus_station.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: NexusToolset) -> None:
        """Hide or unhide plan mode tools based on client capabilities."""
        from nexus_station.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from nexus_station.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if client is not None:
            from nexus_station.telemetry import set_client_info
        ...
        from kosong.chat_provider.nexus import Kimi

        if isinstance(llm.chat_provider, Kimi):
            nexus_stationent = llm.chat_provider.client
            headers = dict(nexus_stationent._custom_headers)  # pyright: ignore[reportPrivateUsage]
