...
from nexus_station.soul import LLMNotSet, wire_send
from nexus_station.soul.dynamic_injection import normalize_history
from nexus_station.soul.message import system_reminder
from nexus_station.utils.logging import logger
from nexus_station.wire.types import BtwBegin, BtwEnd, TextPart
...
def _build_btw_context(soul: NexusSoul, question: str) -> tuple[str, list[Message], _DenyAllToolset]:
    ...
    as ``NexusSoul._step`` so the LLM provider can reuse the prompt cache.
    """
...
async def execute_side_question(
    ...
    Args:
        soul: The NexusSoul instance (for context and chat_provider access).
...
async def run_side_question(soul: NexusSoul, question: str) -> None:
