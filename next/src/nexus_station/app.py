...
from nexus_station.constant import VERSION
from nexus_station.llm import augment_provider_with_env_vars, create_llm, model_display_name
...
from nexus_station.utils.path import shorten_home
from nexus_station.wire import Wire, WireUISide
from nexus_station.wire.types import ApprovalRequest, ApprovalResponse, ContentPart, WireMessage
...
def _patch_session_id(record: dict[str, Any]) -> None:
    ...
    try:
        from nexus_station.soul.toolset import get_session_id
...
async def _refresh_managed_models_silent(config: Config) -> None:
    from nexus_station.auth.platforms import refresh_managed_models
...
class NexusCLI:
    ...
    async def create(
        ...
        *,
        ...
        try:
            from nexus_station.plugin.manager import (
                collect_host_values,
                get_plugins_dir,
        ...
        if telemetry_disabled:
            ...
            from nexus_station.telemetry.sink import EventSink
        ...
        from nexus_station.telemetry import track, track_session_started_once
        from nexus_station.telemetry.crash import install_asyncio_handler, set_phase
    ...
    async def run(
        ...
        """
        Run the NEXUS Station instance without any UI and yield Wire messages directly.
    ...
    async def run_shell(
        ...
        """Run the NEXUS Station instance with shell UI."""
        from nexus_station.ui.shell import Shell, WelcomeInfoItem

        if command is None:
            from nexus_station.ui.shell.update import check_update_gate
    ...
    async def run_print(
        ...
        *,
        ...
        """Run the NEXUS Station instance with print UI."""
        from nexus_station.ui.print import Print
    ...
    async def run_acp(self) -> None:
        """Run the NEXUS Station instance as ACP server."""
        from nexus_station.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the NEXUS Station instance as Wire server over stdio."""
        from nexus_station.wire.server import WireServer
