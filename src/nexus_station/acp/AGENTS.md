...
- **Single-session server**: `KimiCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/nexus_station/app.py`, `src/nexus_station/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/nexus_station/acp/__init__.py`, `src/nexus_station/acp/server.py`.
  - Exposed via the `kimi acp` command in `src/nexus_station/cli/__init__.py`.
