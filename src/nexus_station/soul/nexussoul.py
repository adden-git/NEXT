...
from nexus_station.approval_runtime import (
    ApprovalSource,
    get_current_approval_source_or_none,
    reset_current_approval_source,
    set_current_approval_source,
)
from nexus_station.background import build_active_task_snapshot
from nexus_station.hooks.engine import HookEngine
from nexus_station.llm import ModelCapability
from nexus_station.notifications import (
    NotificationView,
    build_notification_message,
    extract_notification_ids,
)
from nexus_station.skill import Skill, read_skill_text
from nexus_station.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
from nexus_station.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    Soul,
...
from nexus_station.soul.agent import Agent, Runtime
from nexus_station.soul.compaction import (
    CompactionResult,
    SimpleCompaction,
...
from nexus_station.soul.dynamic_injection import (
    DynamicInjection,
    DynamicInjectionProvider,
    normalize_history,
)
from nexus_station.soul.dynamic_injections.afk_mode import AfkModeInjectionProvider
from nexus_station.soul.dynamic_injections.plan_mode import PlanModeInjectionProvider
from nexus_station.soul.message import check_message, system, system_reminder, tool_result_to_message
from nexus_station.soul.slash import registry as soul_slash_registry
from nexus_station.soul.toolset import NexusToolset
from nexus_station.tools.dmail import NAME as SendDMail_NAME
from nexus_station.tools.utils import ToolRejectedError
from nexus_station.utils.logging import logger
from nexus_station.utils.slashcmd import SlashCommand, parse_slash_command_call
...
class NexusSoul:
    ...
    def __init__(
        ...
        *,
        ...
        if self._plan_session_id is not None and self._runtime.session.state.plan_slug is not None:
            from nexus_station.tools.plan.heroes import seed_slug_cache
    ...
    def set_hook_engine(self, engine: HookEngine) -> None:
        ...
        if isinstance(self._agent.toolset, NexusToolset):
    ...
    def _bind_plan_mode_tools(self) -> None:
        """Bind plan mode state to tools that support it."""
        if not isinstance(self._agent.toolset, NexusToolset):
            return
        ...
        from nexus_station.tools.file.write import WriteFile
        ...
        from nexus_station.tools.file.replace import StrReplaceFile
        ...
        from nexus_station.tools.plan import ExitPlanMode
        ...
        from nexus_station.tools.plan.enter import EnterPlanMode
        ...
        from nexus_station.tools.ask_user import AskUserQuestion
    ...
    def _ensure_plan_session_id(self) -> None:
        ...
        if self._plan_session_id is None:
            ...
            from nexus_station.tools.plan.heroes import get_or_create_slug
    ...
    def get_plan_file_path(self) -> Path | None:
        ...
        if self._plan_session_id is None:
            return None
        from nexus_station.tools.plan.heroes import get_plan_file_path
    ...
    def read_current_plan(self) -> str | None:
        ...
        if self._plan_session_id is None:
            return None
        from nexus_station.tools.plan.heroes import read_plan_file
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            from nexus_station.soul.toolset import set_session_id
            ...
            if not command_call:
                ...
                if session.state.custom_title is None:
                    from nexus_station.utils.string import shorten
                    ...
                    if title:
                        from nexus_station.session_state import (
                            load_session_state,
                            save_session_state,
                        )
    ...
    def _make_skill_runner(self, skill: Skill) -> Callable[[NexusSoul, str], None | Awaitable[None]]:
        async def _run_skill(soul: NexusSoul, args: str, *, _skill: Skill = skill) -> None:
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, NexusToolset):
            ...
            try:
                ...
                if loading:
                    from nexus_station.telemetry import track as _track_mcp
        ...
        while True:
            ...
            try:
                ...
                # --- StopFailure hook ---
                from nexus_station.hooks import events as _hook_events
...
class FlowRunner:
    ...
    async def run(self, soul: NexusSoul, args: str) -> None:
        if args.strip():
