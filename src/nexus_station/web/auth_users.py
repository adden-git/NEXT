...
# ═══════════════════════════════════════════════════════════════════════════════
#  Master token verification — station administrator access
#  Token is NEVER stored in code or Git. Loaded from server-side secret only.
# ═══════════════════════════════════════════════════════════════════════════════

_ADMIN_TOKEN_HASH: str | None = None


def _load_admin_hash() -> str | None:
    """Load admin token hash from env var or server-side file.
    Priority: NEXUS_ADMIN_KEY_HASH env > NEXUS_ADMIN_KEY_FILE > None.
    """
    env_hash = os.environ.get("NEXUS_ADMIN_KEY_HASH")
    if env_hash:
        return env_hash.strip()
    key_file = os.environ.get("NEXUS_ADMIN_KEY_FILE", "/etc/nexus/.admin_key")
    p = Path(key_file)
    if p.exists():
...
def verify_admin_token(token: str) -> dict[str, str] | None:
    """Verify master admin token. Bypasses all restrictions including sync."""
    global _ADMIN_TOKEN_HASH
    if _ADMIN_TOKEN_HASH is None:
        _ADMIN_TOKEN_HASH = _load_admin_hash()
    expected = _ADMIN_TOKEN_HASH
    if not expected or not token:
        return None
    token_hash = hashlib.sha1(token.encode()).hexdigest()
    if not hmac.compare_digest(token_hash, expected):
        return None
    return {"username": "station_admin", "role": "master"}
