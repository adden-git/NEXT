...
def kimi(
    ...
    from nexus_station.session import Session
    from nexus_station.ui.shell.startup import ShellStartupProgress
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from nexus_station.utils.path import is_within_directory
    ...
    if _picker_mode:
        ...
        from nexus_station.utils.datetime import format_relative_time
    ...
    if switch_target in ("web", "vis"):
        from nexus_station.utils.logging import restore_stderr
        ...
        from nexus_station.utils.term import ensure_tty_sane
        ...
        if switch_target == "web":
            ...
            from nexus_station.vis.app import run_vis_server
...
def login(
    ...
    from nexus_station.auth.oauth import login_kimi_code
...
def logout(
    ...
    from nexus_station.auth.oauth import logout_kimi_code
    from nexus_station.config import load_config
...
def term(
    ...
    """Run Toad TUI backed by NEXUS Station ACP server."""
...
@cli.command()
def acp():
    """Run NEXUS Station ACP server."""
    from nexus_station.acp import acp_main
...
if __name__ == "__main__":
    ...
    if "nexus_station.cli" not in sys.modules:
        sys.modules["nexus_station.cli"] = sys.modules[__name__]
