...
def get_build_sha() -> str:
    ...
    Priority:
    1. NEXUS_BUILD_SHA environment variable (dev / CI override)
    ...
    """
    if build_id := os.environ.get("NEXUS_BUILD_SHA", "").strip():
        return build_id

    try:
        from nexus_station._build_info import BUILD_SHA  # type: ignore[reportMissingImports]
