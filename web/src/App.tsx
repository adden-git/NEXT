...
import { Sidebar, SidebarSimple, Shield } from "@/components/nexus-icons";
...
import { consumeAuthTokenFromUrl, getAuthToken, setAuthToken, getAuthHeader } from "./lib/auth";

function SessionSyncGuard() {
  const [sync, setSync] = useState<{expired: boolean; days_remaining: number} | null>(null);
  useEffect(() => {
    fetch("/api/config/license", { headers: getAuthHeader() }).then(r => r.json()).then(setSync).catch(() => setSync(null));
  }, []);
  if (!sync) return null;
  if (sync.expired) {
    return (
  ...
  const handleSessionStatus = useCallback(
    (status: SessionStatus) => {
      ...
      if (reason === "config_update") {
        ...
        window.dispatchEvent(new Event("nexus:config-update"));
      }
