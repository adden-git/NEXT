...
const TYPE_CONFIG: Record<string, { label: string; icon: React.ElementType; description: string; autoNote?: string; manualNote?: string }> = {
  agents: {
    label: "AGENTS.md",
    icon: Shield,
    description: "Инструкции проекта. NEXUS Station ищет от .git до work_dir и мержит все найденные.",
    autoNote: "Авто-дискавери: от project_root до work_dir",
  },
  skill: {
    label: "Навыки (Skills)",
    icon: Puzzle,
    description: "Модульные инструкции в папках .nexus/skills/*/, .claude/skills/*/, .codex/skills/*/, .agents/skills/*/. NEXUS Station подхватывает автоматически.",
    autoNote: "Авто-дискавери: .nexus/skills/*/SKILL.md, .claude/skills/*/SKILL.md, .codex/skills/*/SKILL.md, .agents/skills/*/SKILL.md",
  },
  agent: {
    label: "Агенты (.md)",
    icon: FileCode,
    description: "Markdown-файлы в .nexus/agents/*.md — legacy, NEXUS Station их не видит.",
    manualNote: "NEXUS Station использует YAML-агенты (--agent-file), не .md",
  },
  prompt: {
    label: "Промпты",
    icon: MessageSquare,
    description: "Файлы в .nexus/prompts/*.md — остались от VS Code extension.",
    manualNote: "NEXUS Station не подхватывает .md промпты автоматически",
  },
  hook: {
    label: "Хуки",
    icon: Terminal,
    description: "Скрипты в .nexus/hooks/*.sh — для автоматизации approval'ов.",
    manualNote: "Настраиваются в ~/.nexus/config.toml → [[hooks]]",
  },
  plugin: {
    label: "Плагины",
    icon: Plug,
    description: "Папка .nexus/plugins/ — устанавливаются через 'nexus plugin install'.",
    manualNote: "Используйте 'nexus plugin install <name>' в терминале",
  },
  instruction: {
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <p className="text-[10px] text-muted-foreground">
          Для NEXUS K2.5/K2.6: default 32 768, для K2 Thinking: 64 000
        </p>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md bg-muted/40 p-2.5 space-y-1.5">
        <div className="flex items-start gap-2">
          ...
          <div className="text-[10px] text-muted-foreground leading-relaxed space-y-1">
            ...
            <p>
              NEXUS Station ищет AGENTS.md от project_root (ближайший .git) до work_dir.
              Skills подхватываются из .nexus/skills/*/, .agents/skills/*/, .claude/skills/*/, .codex/skills/*/
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-[10px] font-semibold text-green-600 dark:text-green-400 uppercase tracking-wider flex items-center gap-1">
          ...
          Автоматически подхватывается NEXUS Station
        </p>
      ...
      {files.length === 0 && (
        <div className="rounded-lg border border-dashed p-4 text-center space-y-3">
          ...
          <div className="text-[10px] text-muted-foreground text-left space-y-2">
            <p><strong>Что подхватывает NEXUS Station автоматически:</strong></p>
            <ul className="list-disc pl-4 space-y-1">
              <li><code>AGENTS.md</code> — правила поведения AI (ищется от .git до work_dir)</li>
              <li><code>.nexus/AGENTS.md</code> — дополнительные правила</li>
              <li><code>.nexus/skills/*/SKILL.md</code> — навыки проекта</li>
            ...
            <p><strong>Не подхватывается NEXUS Station:</strong></p>
            <ul className="list-disc pl-4 space-y-1">
              <li><code>.nexus/prompts/*.md</code> — legacy VS Code extension</li>
              <li><code>.nexus/agents/*.md</code> — NEXUS Station использует YAML-агенты</li>
              <li><code>.nexus/hooks/*.sh</code> — настраиваются в config.toml</li>
              <li><code>.nexus/plugins/</code> — устанавливаются через CLI команду</li>
            </ul>
          </div>
    ...
    <Dialog open={open} onOpenChange={(newOpen) => {
      ...
      <DialogContent className="max-w-lg max-h-[85dvh] p-0 gap-0 flex flex-col" showCloseButton={false}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-muted/30 shrink-0">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold flex items-center gap-2"><Settings className="size-5" /> Параметры сессии</h2>
            {dirty && (
          ...
          <div className="space-y-6 pb-8">
            ...
            <section>
              <h3 className="text-sm font-medium text-muted-foreground mb-3 flex items-center gap-2">
                <Shield className="size-4" /> Guardian AI (двойная проверка)
              </h3>
              {guardianLoading ? (
            ...
            <section>
              <h3 className="text-sm font-medium text-muted-foreground mb-3 flex items-center gap-2">
                <FileCode className="size-4" /> Git diff
              </h3>
              {gitDiff === null ? (
                <div className="text-muted-foreground text-sm">Загрузка...</div>
              ) : !gitDiff.is_git_repo ? (
